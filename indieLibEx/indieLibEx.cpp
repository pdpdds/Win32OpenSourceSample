// indieLibEx.cpp : 콘솔 응용 프로그램에 대한 진입점을 정의합니다.
//
...
#ifdef _DEBUG
#pragma comment(lib, "IndieLibd.lib")
#else
#pragma comment(lib, "IndieLib.lib")
#endif
...
public:
	...
	static bool setWorkingPathFromExe(const TCHAR* relpath);
...
static TCHAR g_appDir[appDirChars];

bool WorkingPathSetup::setWorkingPathFromExe(const TCHAR* relpathFromExe) {
	...
	TCHAR totalPath[8042];
	_tcscpy(totalPath, g_appDir);
	if (relpathFromExe) {
		_tcscat(totalPath, relpathFromExe);
	}
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	if (!mI->_surfaceManager->add(mSurfaceCave, "indieExRes/cave.png", IND_ALPHA, IND_32)) return 0;

	// Loading cave (first plane)
	IND_Surface *mSurfaceCaveFirstPlane = IND_Surface::newSurface();
	if (!mI->_surfaceManager->add(mSurfaceCaveFirstPlane, "indieExRes/cave_near.png", IND_ALPHA, IND_32)) return 0;

	// Loading sky
	IND_Surface *mSurfaceSky = IND_Surface::newSurface();
	if (!mI->_surfaceManager->add(mSurfaceSky, "indieExRes/sky.jpg", IND_OPAQUE, IND_32)) return 0;

	// Font
	IND_Font *mFontSmall = IND_Font::newFont();
	if (!mI->_fontManager->add(mFontSmall, "indieExRes/font_small.png", "indieExRes/font_small.xml", IND_ALPHA, IND_32)) return 0;
